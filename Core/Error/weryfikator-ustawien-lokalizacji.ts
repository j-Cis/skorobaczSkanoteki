 ...
 * 
*/
export function SprawdzaUstawienia_GdzieZadania():void {
  const CZY: RecordStringBoolean = <RecordStringBoolean>{};
  CZY.ISTNIEJE_PLIK = FS.existsSync(PATH.resolve(Deno.cwd(), "Config", "GDZIE_ZADANIA.toml.txt"), { isReadable: true, isFile: true });  
  ...
  try {
    ...
    const OBIEKT:RecordStringUnknown = TOML.parse(Deno.readTextFileSync(PATH.resolve(Deno.cwd(), "Config", "GDZIE_ZADANIA.toml.txt")));
    ...
    if(CZY.ISTNIEJE_PLIK){
      Deno.removeSync(PATH.resolve(Deno.cwd(), "Config", "GDZIE_ZADANIA.toml.txt"))
    }
    Deno.writeTextFileSync(PATH.resolve(Deno.cwd(), "Config", "GDZIE_ZADANIA.toml.txt"),TOML.stringify({ ZADANIA_W_FOLDERZE:"Batch", ARCHIWUM_W_FOLDERZE:"Archive"},{keyAlignment:true}));
    console.log(C.bgRgb8(C.black(C.bold("PRZYWRÓCONO DOMYŚLNE USTAWIENIA 'GDZIE_ZADANIA > FOLDER_Z_ZADANIAMI' ORAZ 'GDZIE_ZADANIA > FOLDER_Z_ARCHIWUM' , BO ALBO BYŁY NIE CZYTELNE, ALBO USUNIĘTE")),231));
  } finally {
    const ADRES:RecordStringString=<RecordStringString>{};
    const OBIEKT:RecordStringUnknown = TOML.parse(Deno.readTextFileSync(PATH.resolve(Deno.cwd(), "Config", "GDZIE_ZADANIA.toml.txt")));
    ADRES.FOLDER_Z_ZADANIAMI = PATH.resolve(Deno.cwd(),`${OBIEKT.ZADANIA_W_FOLDERZE}`);
...
SprawdzaUstawienia_GdzieZadania();
